...
(async () => {
    ...
    await page.goto("https://www.auchan.fr/oeufs-produits-laitiers/desserts-compotes/ca-n0103", { waitUntil: ['networkidle2'] });
    ...
    //     const nextProductLinkList = await page.evaluate((selector, i) => {
    //         const element = document.querySelectorAll(selector)[i];
    //         if(element) {
    //           element.scrollIntoView();
    //         }
    //       }, selector, i);
    //     if(nextProductLinkList){
    //         productLinkList = productLinkList.concat(nextProductLinkList);
    //     }else{
    //         break;
    //     }
    ...
    if (productLinkList.length != 0) {
        for (const { link } of productLinkList) {
            ...
            const produit = new Produit({ distributeur: distributeur, lien: link });
            await produit.save();
            console.log('lien ajouté en BD');
        }
    }
