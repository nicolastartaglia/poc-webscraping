...
const fichier = fs.readFileSync(path.resolve(__dirname, "./libelles_produits"), 'utf-8');
...
(async () => {
    ...
    const Produit = mongoose.model('AuchanLibelleProduit', distributeurSchema);
    ...
    // Noyelles-Godault (le plus grand magasin auchan)
    ...
    for (const recherche of recherches) {
        ...
        if (doc == null) {
            ...
            if (await page.$('span.no-result__category') !== null) {
                const produit = new Produit({ job: job, distributeur: distributeur, recherche: recherche, title: "pas trouvé", prix: "pas trouvé" });
                await produit.save();
            } else {
                ...
                if (productLinkList.length != 0) {
                    for (const { link } of productLinkList) {
                        ...
                        const blockTitle = await page.waitForSelector('.site-breadcrumb__title.site-breadcrumb__title--ellipsis');
                        const title = await page.evaluate(el => el.innerText, blockTitle);
                        const titleBD = await Produit.findOne({ titre: title });
                        if (titleBD == null) {
